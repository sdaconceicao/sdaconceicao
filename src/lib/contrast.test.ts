...
describe("the spotlight contrast budget", () => {
  /**
   * The glow composites over the page and lowers every text ratio beneath it,
   * so its alpha is bounded by the weakest text token, not chosen by eye.
   *
   * Blending happens in GAMMA-ENCODED sRGB, because that is what source-over
   * compositing does. Averaging relative luminances instead (a tempting
   * shortcut) models linear-light compositing and overstates the damage by
   * roughly a full ratio point -- it would reject alphas that are actually fine.
   */
  const compositeRatio = (over: string, tint: string, alpha: number): number => {
    const bg = parseHex(over);
    const tn = parseHex(tint);
    const mix = (a: number, b: number) => Math.round(a * (1 - alpha) + b * alpha);
    const blended = `#${[mix(bg.r, tn.r), mix(bg.g, tn.g), mix(bg.b, tn.b)]
      .map((v) => v.toString(16).padStart(2, "0"))
      .join("")}`;
    return contrastRatio("#8B8285", blended);
  };

  it("keeps --fg-dim at AA with headroom at the shipped 18%", () => {
    const ratio = compositeRatio(BG, "#A4161A", 0.18);
    expect(ratio).toBeGreaterThanOrEqual(4.5);
    expect(ratio).toBeCloseTo(4.91, 1);
  });

  it("still passes at 26% but with almost no margin, which is why we ship 18%", () => {
    expect(compositeRatio(BG, "#A4161A", 0.26)).toBeCloseTo(4.66, 1);
  });

  it("drops --fg-dim below AA at 40%", () => {
    expect(compositeRatio(BG, "#A4161A", 0.4)).toBeLessThan(4.5);
  });

  it("would fail immediately if the glow used the light accent instead of the brand red", () => {
    // #FF5A5A is far brighter; at the same alpha it blows the budget outright.
    expect(compositeRatio(BG, "#FF5A5A", 0.18)).toBeLessThan(4.5);
  });

  it("ships the 18% figure in tokens.css", () => {
    expect(tokens).toMatch(/color-mix\(in oklab, var\(--red-600\) 18%, transparent\)/);
  });
});
